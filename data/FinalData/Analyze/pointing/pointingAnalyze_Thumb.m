...
for j=1:length([listing.isdir])%know the length of 
    if strfind(listing(j).name,'Jackie_resultOfPointing.xls')~=0%find the document we want
...
for j=1:length([listing.isdir])%know the length of 
    if strfind(listing(j).name,'YS_resultOfPointing.xls')~=0%find the document we want
        filename = convertCharsToStrings([listing(j).folder '\' listing(j).name]);
        [YScolOne colTwo colThree] = xlsread(filename);%get the first column number
    end
end
for j=1:length([listing.isdir])%know the length of 
    if strfind(listing(j).name,'Man_resultOfPointing.xls')~=0%find the document we want
        filename = convertCharsToStrings([listing(j).folder '\' listing(j).name]);
        [MancolOne colTwo colThree] = xlsread(filename);%get the first column number
    end
end
for j=1:length([listing.isdir])%know the length of 
    if strfind(listing(j).name,'Raymond_resultOfPointing.xls')~=0%find the document we want
        filename = convertCharsToStrings([listing(j).folder '\' listing(j).name]);
