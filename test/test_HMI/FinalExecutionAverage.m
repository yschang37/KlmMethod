...
for j=1:length([listing.isdir])%know the length of 
    if strfind(listing(j).name,'ExecrtionTime.xls')~=0%find the document we want
        filename = convertCharsToStrings([listing(j).folder '\' listing(j).name]);
        colOne = xlsread(filename);%get the first column number
...
xlswrite('FinalExecutionAverage',c);
% 
